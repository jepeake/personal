...
*The code that implements these architectures are relatively small & simple. For example, the 70B parameters Llama 2 model.py file contains only around ~500 lines of code, while requiring ~130 GB of memory to store the weights.*
...
*Imagine maximising intelligence towards infinite, such that a being has the ultimate, maximal intelligence possible. What would this being look like? I would suggest that this being would be **all knowing.** Therefore - it would understand the true nature of the universe & everything in it.*
...
*Given this definition - how can a relation be found from intelligence to data, code, & compute.*

*Imagine a model that has been trained on a vast amount of data & compute - & has a simple code implementation.*
...
***Therefore: as the amount of data increases, there is a better representation of the true world for the model to learn from & therefore the model can encode a better model of the world into it’s architecture, & has more intelligence. The code & model architecture remain the same - but the model which is encoded upon it defines it’s capabilities.**